...
const StoreCart = () => {
  ...
  // TODO: integrate with real cart state from StoreMenu
  ...
  const deliveryFee = orderType === "delivery" ? Number(store?.delivery_fee || 5) : 0;
  ...
  const handleWhatsAppOrder = () => {
    ...
    const itemsText = items.map((item) => {
      let line = `• ${item.quantity}x ${item.name} (${item.sizeName})`;
      if (item.addons.length > 0) line += ` + ${item.addons.join(", ")}`;
      line += ` — R$ ${(item.price * item.quantity).toFixed(2).replace(".", ",")}`;
      return line;
    }).join("\n");
    ...
    <div className="min-h-screen bg-background text-foreground">
      <header className="sticky top-0 z-50 border-b border-border bg-background/95 backdrop-blur-md">
        <div className="flex items-center justify-between h-14 px-4 lg:px-8">
          ...
          <Link to={`/r/${slug}`} className="text-sm text-muted-foreground hover:text-foreground transition-colors">
            ← Voltar ao cardápio
          </Link>
        </div>
      </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                items.map((item) => (
                  <div key={item.id} className="flex items-center gap-4 p-4 bg-card border border-border rounded-lg">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">{item.sizeName}{item.addons.length > 0 ? ` · ${item.addons.join(", ")}` : ""}</p>
                    </div>
                    ...
                    <div className="flex items-center gap-1 bg-secondary rounded-md">
                      <button onClick={() => updateQty(item.id, -1)} className="px-2 py-1 text-foreground hover:text-primary"><Minus className="h-3 w-3" /></button>
                      <span className="text-sm font-medium w-6 text-center text-foreground">{item.quantity}</span>
                      <button onClick={() => updateQty(item.id, 1)} className="px-2 py-1 text-foreground hover:text-primary"><Plus className="h-3 w-3" /></button>
                    </div>
                    <button onClick={() => removeItem(item.id)} className="text-destructive hover:text-destructive/80"><Trash2 className="h-3.5 w-3.5" /></button>
                  </div>
                ))
              )}
            </div>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-20 space-y-4">
              ...
              <Button onClick={handleWhatsAppOrder} disabled={items.length === 0} className="w-full bg-primary text-primary-foreground hover:bg-primary/90 py-6 text-base font-semibold">
